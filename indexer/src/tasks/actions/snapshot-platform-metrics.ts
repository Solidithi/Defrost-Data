...
import { convertOnchainAmountToOffChainAmount } from "../../utils/converter";
	...
	try {
		...
		try {
			...
			for (const pool of pools) {
				logger.info(`Processing pool with asset address ${pool.v_asset_address}`);
				...
				try {
					...
					const offchainAmount = ethers.formatUnits(totalStakedBigInt, decimals);
					...
					try {
						...
						logger.info(
							`Price for ${id} in USD: ${priceInUSD}`
						);
			...
			logger.error(
				`Error fetching pools: ${error}`
			);

		}

		console.log(
			"Total Staked Amount Off Chain in Pools: ", poolUSDAmounts
		);
		const totalStakedAmount: number = poolUSDAmounts.reduce((acc, amount) => acc + amount, 0);
		console.log(
			"Total Staked Amount Off Chain in Pools: ", totalStakedAmount
		);
		...
		logger.info(`Successfully inserted platform metrics snapshot with ID: ${insertData.id}`);
